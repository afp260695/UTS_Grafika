...
typedef struct rgb {
    ...
    static const rgb RED2;
    static const rgb MAGENTA;
    static const rgb MAGENTA2;
...
const rgb rgb::RED2 = {254, 0, 0};
const rgb rgb::MAGENTA = {255, 0, 255};
const rgb rgb::MAGENTA2 = {255, 0, 254};
...
void floodFill2(int x, int y, rgb batas1, rgb batas2, rgb warna){
    if((x>=0 && x<WIDTH) && (y>=0 && y<HEIGHT)){
        if(
            !(
                (pixelMatrix[x][y] == batas1) ||
                (pixelMatrix[x][y] == batas2) ||
                (pixelMatrix[x][y] == warna)
            )
        ){
            pixelMatrix[x][y] = warna;
            floodFill2(x,y+1, batas1, batas2, warna);
            floodFill2(x+1,y, batas1, batas2, warna);
            floodFill2(x,y-1, batas1, batas2, warna);
            floodFill2(x-1,y, batas1, batas2, warna);
        }
    }
...
void drawPohon(int x, int y, int size) {
    drawLine(rgb::WHITE, x+size-1-(size-1)/2, y, x, y+size-1);
    drawLine(rgb::WHITE, x+(size-1)/2, y, x+size-1, y+size-1);
    drawLine(rgb::WHITE, x, y+size-1, x+size-1, y+size-1);
    floodFill(x+(size-1)/2, y+(size-1)/2, rgb::WHITE, rgb::GREEN);
    ...
    for (int i = 0; i < MAP_WIDTH; ++i)
    {
        for (int j = 0; j < MAP_HEIGHT; ++j)
        {
            if (pixelMatrix[i][j] == rgb::BLACK)
            {
                ...
                floodFill(i, j, rgb::WHITE, rgb::GRAY);
                hue += 10;
                if (hue > 360)
                {
                    hue = 0;
...
void drawBadan(int x, int y, int scaling, rgb warna, rgb batas){
    ...
    floodFill(x+1, y+3*scaling, batas, warna);
}
